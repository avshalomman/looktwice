...
def calc_scores(results):
    ...
    for i in range(number_answered_samples):
        answers[i] = {
            "q0_i0": extract_answer(results[i*4]['pred'], results[i*4]['type']),
            "q0_i1": extract_answer(results[i*4+1]['pred'], results[i*4+1]['type']),
            "q1_i0": extract_answer(results[i*4+2]['pred'], results[i*4+2]['type']),
            "q1_i1": extract_answer(results[i*4+3]['pred'], results[i*4+3]['type'])
        }
